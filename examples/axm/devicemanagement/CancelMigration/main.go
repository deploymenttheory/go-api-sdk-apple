...
func main() {
	fmt.Println("=== Apple Business Manager - Cancel a Device Management Service Migration ===")
...
-----END EC PRIVATE KEY-----`
	...
	// CANCEL_MDM_MIGRATION takes neither an mdmServer relationship nor metadata.
	// Serial numbers that are not undergoing a migration are reported as failures in
	// the activity log, not as an error on this call.
	...
	if err != nil {
		log.Fatalf("Error cancelling device management service migration: %v", err)
	}

	fmt.Printf("Cancellation activity created:\n")
